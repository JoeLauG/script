...
{
    Remove-Item $logfile
    Add-Content -Path $logfile -Value "Start running" -Encoding UTF8
}
#Remove-Item $logfile
...
foreach($line in Get-Content $inputpath) {
    ...
    #$lastindex = $line.LastIndexOf("AND MEMBER_NUMBER='")
    #$endlastindex= $line.LastIndexOf("' AND MOVEMENT_TYPE")
    #$len = "AND MEMBER_NUMBER='".Length

    #if($lastindex -ne -1){
    #    $line = $line.Substring($lastindex + $len,  $endlastindex - $lastindex - $len)
    #}
...
foreach($file in $chi_files)
{
    ...
    $fileContent = get-content  -Encoding "UTF8"  $filepath -raw
    ...
    $fileContents = $fileContent -split "`r`n"
    $filteredcontent =  ""
    
    $abc = $fileContents | Where-Object {
        $line = $_ 
        $removeline = $false
        ...
        {
            if($line -ne ""){
                $filteredcontent = $filteredcontent + $line + "`r`n"
            }
        }
        -not $removeline
    }
    ...
    if($null -ne $filteredcontent){
        ...
        #[System.IO.File]::WriteAllLines($destpath , $filteredcontent, [System.Text.Encoding]::UTF8)
    }else{
        #write-host "NULL"
...
#$table | Format-Table FileName,Num_of_Record,Exec_Duration –AutoSize
